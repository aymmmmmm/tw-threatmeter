...
**Bug 修复**
- 修复色彩饱和度设置仅作用于其他玩家的条，玩家自己的条和 AGRO 条现在也受饱和度影响
- 移除玩家条的硬编码颜色覆盖（原先调节饱和度后被 `SetStatusBarColor(1, 0.2, 0.2, 1)` 覆盖）
- 修复全屏告警发光纹理使用固定像素尺寸，改用 `SetAllPoints` 自适应任意分辨率和 UI 缩放

**改进**
- 打开设置面板时自动生成预览数据，可实时预览饱和度、柔和色等外观调节效果
- 新增色彩饱和度滑块（1-20，默认 10）
- 新增 `/twt saturation [1-20]` 斜杠命令
